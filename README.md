...
>
> Just describe what you want in plain language, e.g. *"run my tests on GhostBSD"* or *"check that my project builds on GhostBSD aarch64"*, and the agent generates a correct, ready-to-commit `test.yml`. It will:
>
...
Use this action to run your CI in GhostBSD.

The github workflow only supports Ubuntu, Windows and MacOS. But what if you need to use GhostBSD?
...
jobs:
  test:
    ...
    name: A job to run test in GhostBSD
    env:
...
It uses [the GhostBSD 26.1](conf/default.release.conf) by default, you can use `release` option to use another version of GhostBSD:
...
Here `release: "26"` runs the newest `26.x` release of GhostBSD. Each part you give has to match in full, so a release that does not exist fails the job instead of quietly falling back to another one.
...
We use Qemu to run the GhostBSD VM.
